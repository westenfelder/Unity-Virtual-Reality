...
public class Main : MonoBehaviour
{
    ...
    Connection createConnection(GameObject node1, GameObject node2)
    {
        ...
        connection.connectionObject.GetComponent<Renderer>().material.color = new Color32(233, 79, 55, 150);
    ...
    void Start()
    {
        ...
        {
            //make sure position vector is unique
            bool unique = false;
            Vector3 pos = randomPosVector();
            while (!unique)
            {
                unique = true;
                for (int j = 0; j < nodeList.Count; j++)
                {
                    Vector3 otherPos = nodeList[j].transform.localPosition;
                    if ((Math.Abs(pos.x - otherPos.x) < 0.5) || (Math.Abs(pos.y - otherPos.y) < 0.5) || (Math.Abs(pos.z - otherPos.z) < 0.5))
                    {
                        unique = false;
                        pos = randomPosVector();
                    }
            ...
            GameObject node = createNode(pos, randomSizeVector());
    ...
    void FixedUpdate()
    {
        ...
        {
            nodeList[i].GetComponent<Rigidbody>().velocity = Vector3.zero;
            nodeList[i].GetComponent<Rigidbody>().angularVelocity = Vector3.zero;
